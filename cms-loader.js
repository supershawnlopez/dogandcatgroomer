...
function buildHoursText(data) {
  const fromDay = data.business_days_from;
  const toDay = data.business_days_to;
  const open = data.business_open;
  const close = data.business_close;

  if (fromDay && toDay && open && close) {
    return `${fromDay} to ${toDay} · ${open}–${close}`;
  }

  return data.phone_note || '';
}
...
async function loadContact() {
  ...
  set('phone-note', buildHoursText(data));
